...
contract Atlaten is IAtlaten, ERC721Enumerable, Ownable {
    using Strings for uint256;
    // Base Token URI
    ...
    constructor(
        ...
        revenueAddress = payable(msg.sender);
    }
    ...
    function adminMint(uint256 _tokenId) external onlyOwner {
        require(!_exists(_tokenId), "Token Id exists already");
        require(_tokenId > 0, "Token Id cannot be 0");
        require(totalSupply() + 1 <= MAX_SUPPLY, "NFT: max supply reached");
